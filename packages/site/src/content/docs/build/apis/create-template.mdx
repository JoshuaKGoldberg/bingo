...
These must be manually provided by users, such as through a [configuration file](/configuration#settings)
...
export default createTemplate({
	...
	prepare({ take }) {
		return {
			author: take(inputFromScript, { command: "npm whoami" }),
		};
	},
	produce() {
		// ...
...
export default createTemplate({
	...
	prepare({ runner }) {
		const npm = take(inputFromScript, { command: "npm whoami" });
		const github = take(inputFromScript, { command: "gh api user -q .login" });
